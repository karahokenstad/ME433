...
int main() {
    ...
    while (1) {
        ...
        ssd1306_clear();
        drawString(0,0,"Hello - ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        ssd1306_update();
        count = _CP0_GET_COUNT();
        time = 24000000/(float)count;
        sprintf(m,"FPS: %f",time);
        drawString(0,20,m);
        ssd1306_update();
...
void blink() {
    ...
    while(_CP0_GET_COUNT() < 12000000) {} // 0.5 s
    LATAbits.LATA4 = 0; //led off
    while(_CP0_GET_COUNT() < 24000000) {} // 1 s
}
