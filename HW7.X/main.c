...
int main() {
    ...
    #define dt 0.01 // time between measurements
    #define a 0.5 // weight between gyro and accelerometer
    float thax[NUM_DATA_PNTS], thay[NUM_DATA_PNTS];
    float thgx[NUM_DATA_PNTS], thgy[NUM_DATA_PNTS];
    float thestx[NUM_DATA_PNTS], thesty[NUM_DATA_PNTS];
    thgx[0] = 0;
    thgy[0] = 0;
    ...
    while (1) {
        ...
        for (i=0; i<NUM_DATA_PNTS; i++){
            ...
            thax[i] = atan2f(ax[i],az[i]);
            if (i != 0) {
                thgx[i] = gx[i]*dt + thestx[i-1];
            }
            thestx[i] = a*thax[i] + (1-a)*thgx[i]; // estimated theta x
            
            thay[i] = atan2f(ay[i],az[i]); 
            if (i != 0) {
                thgy[i] = gy[i]*dt + thesty[i-1];
            }
            thesty[i] = a*thay[i] + (1-a)*thgy[i]; // estimated theta y
        ...
        // print data
        for (i=0; i<NUM_DATA_PNTS; i++){
            sprintf(m_out,"%d %f %f %f %f %f %f\r\n",NUM_DATA_PNTS-i,ax[i],ay[i],az[i],thestx[i],thesty[i],temp[i]);
            WriteUART1(m_out);
        }
